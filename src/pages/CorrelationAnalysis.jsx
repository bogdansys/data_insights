...
const CorrelationAnalysis = ({ data }) => {
    ...
    for (let i = 0; i < selectedColumns.length; i++) {
      for (let j = i; j < selectedColumns.length; j++) {
        const correlation =
          i === j
            ? 1
            : calculateCorrelation(selectedColumns[i], selectedColumns[j]);
        matrix.push({
          x: selectedColumns[i],
          y: selectedColumns[j],
          value: correlation,
  ...
  const calculateCorrelation = (col1, col2) => {
    ...
    const values1 = data
      .slice(1)
      .map((row) => parseFloat(row[index1]))
      .filter((val) => !isNaN(val));
    const values2 = data
      .slice(1)
      .map((row) => parseFloat(row[index2]))
      .filter((val) => !isNaN(val));
    ...
    const correlation =
      deviation1.reduce((sum, val, i) => sum + val * deviation2[i], 0) /
      Math.sqrt(sum1 * sum2);
            ...
            <Scatter data={correlationData} fill="#8884d8">
              {correlationData.map((entry, index) => (
                <Cell
                  ...
                  fill={
                    entry.value > 0
                      ? `rgb(0, ${Math.floor(entry.value * 255)}, 0)`
                      : `rgb(${Math.floor(-entry.value * 255)}, 0, 0)`
                  }
                />
              ))}
            </Scatter>

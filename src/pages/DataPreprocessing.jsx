      ...
      switch (method) {
        case "remove_missing":
          newData = newData.filter(
            (row, index) => index === 0 || row[columnIndex] !== "",
          );
          break;
        case "fill_mean":
        case "fill_median":
        case "fill_mode":
          const columnData = newData
            .slice(1)
          ...
          newData = newData.map((row, index) => {
            ...
            if (!isNaN(value)) {
              const normalizedValue =
                ((value - minVal) / (maxVal - minVal)) * (max - min) + min;
    ...
    <div className="space-y-4">
      {preprocessingTasks.map((task, index) => (
        <div key={index} className="flex space-x-2">
          <Select
            onValueChange={(value) =>
              updatePreprocessingTask(index, "column", value)
            }
          >
            <SelectTrigger>
          ...
          <Select
            onValueChange={(value) =>
              updatePreprocessingTask(index, "method", value)
            }
          >
            <SelectTrigger>
            ...
            <SelectContent>
              <SelectItem value="remove_missing">
      ...
      {preprocessingTasks.some((task) => task.method === "normalize") && (
        <div className="space-y-2">
          ...
          />
          <div>
            Range: {normalizationRange[0]} - {normalizationRange[1]}
          </div>
        </div>
      )}

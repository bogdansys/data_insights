...
const DataQualityAssessment = ({ data }) => {
  ...
  const assessDataQuality = (data) => {
    ...
    data[0].forEach((header, columnIndex) => {
      ...
      if (missingCount > 0) {
        issues.push(
          `Column "${header}" has ${missingCount} missing values (${((missingCount / columnData.length) * 100).toFixed(2)}%)`,
        );
      }
    ...
    data[0].forEach((header, columnIndex) => {
      ...
      if (columnData.length > 0) {
        ...
        const outliers = columnData.filter(
          (val) => val < lowerBound || val > upperBound,
        );
        if (outliers.length > 0) {
          issues.push(
            `Column "${header}" has ${outliers.length} potential outliers`,
          );
        }
      }
    });
      ...
      <Card>
        ...
        <CardContent>
          ...
          <p className="mt-2 text-sm text-gray-600">
            {dataCompleteness.toFixed(2)}% of cells contain data
          </p>
        </CardContent>
      </Card>
        ...
        <Card>
          ...
          <CardContent>
            <ul className="list-disc pl-5">
              ...
              <li>
                Investigate and potentially transform columns with inconsistent
                data types
              </li>
              <li>
